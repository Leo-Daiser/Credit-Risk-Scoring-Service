...
Web-интерфейс реализует пять основных маршрутов:

- dashboard с фактической историей решений и состоянием batch queue;
- пользовательский кредитный калькулятор, opt-in ML-оценку по короткой анкете и
  экспертный скоринг versioned JSON payload;
...
Расчёт платежа и доступной суммы выполняется в браузере. Поля короткой анкеты
передаются в backend только после явного подтверждения запуска ML-оценки, после чего
запрос и результат сохраняются в PostgreSQL audit log. Такая анкета покрывает только
часть полного feature-контракта, поэтому её результат является предварительным, а не
кредитным решением или офертой.
