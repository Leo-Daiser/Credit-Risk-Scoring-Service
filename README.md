...
1. Пользователь открывает `/assessment` и проходит четыре коротких шага с примерными данными.
2. Получает ML-derived профиль Riskline, понятные сильные и ограничивающие факторы.
3. Проверяет реалистичные сценарии изменения суммы, срока и текущей нагрузки.
...
Калькулятор `/score` остаётся отдельным вспомогательным инструментом. Его значения
передаются в оценку только в памяти открытой страницы и не сохраняются в browser storage.
...
Скрипт сначала запускает `python -m src.cli prepare-local-ml`, проверяет реальный
локальный artifact публичной модели, затем поднимает Compose и выполняет `verify-demo`.
Если artifact отсутствует, команда обучает его только из настроенного локального
датасета; без такого источника запуск завершается явной ошибкой. Ручные команды и
проверка HMAC описаны в [`docs/deployment_demo.md`](docs/deployment_demo.md).
