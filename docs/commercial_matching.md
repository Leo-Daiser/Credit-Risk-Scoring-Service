...
Владение событиями разделено: backend фиксирует получение подбора, завершение
модельного расчёта, генерацию офферов и создание клика; frontend фиксирует только
фактические просмотры результата, блока улучшений, рекомендованной карточки,
no-offer состояния и партнёрского диалога. Frontend дедуплицирует просмотры по
`anonymous_profile_id`, поэтому повторный React render не увеличивает воронку.
